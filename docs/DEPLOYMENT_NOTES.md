...
3. This creates a `docker-compose.override.yml` that adds `cpuset: "0-15"` to all `server-*` services.
...
2. **NAT Hairpinning for LAN Players**: If players are on the same local network as the server, but trying to connect via your public IP, they might fail to connect.
   *Fix*: Ensure your router supports **NAT Loopback / Hairpinning**, or have local players connect directly to the server's local LAN IP (e.g., `192.168.x.x`).
