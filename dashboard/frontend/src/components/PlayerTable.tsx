...
export function PlayerTable({ players, onBan, onKick, selectedSteamIds, onToggleSelect }: PlayerTableProps) {
        ...
        <table className="min-w-full divide-y divide-th-border-m text-left text-sm">
          <thead className="bg-th-surface-s/50 text-xs uppercase tracking-[0.2em] text-th-text-m">
            <tr>
              ...
              {[
                ...
                <th key={key} className="px-3 py-2 font-medium" aria-sort={sortKey === key ? (ascending ? 'ascending' : 'descending') : 'none'}>
                  <button type="button" onClick={() => toggleSort(key as SortKey)} className="inline-flex min-h-11 items-center gap-2 px-2 transition-colors hover:text-th-text-s">
                    {label}
          ...
          <tbody className="divide-y divide-th-border-m/80">
            {filtered.length > 0 ? (
                ...
                <tr key={player.steamId} className="transition-colors hover:bg-th-surface-s/50">
                  {onToggleSelect && (
                    <td className="w-10 px-3 py-4">
                      <input
                        type="checkbox"
                        checked={selectedSteamIds?.has(player.steamId) ?? false}
                        onChange={() => onToggleSelect(player.steamId)}
              ...
              <tr>
                <td colSpan={onToggleSelect ? 6 : 5} className="px-5 py-12 text-center text-th-text-m">
