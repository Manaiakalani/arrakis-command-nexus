...
export default function PlayersPage() {
  ...
  const players = useApiSWR('api/players', () => apiClient.getPlayers(), { refreshInterval: 10_000, initialData: [] });
  const playerPositions = useApiSWR('api/players/positions', () => apiClient.getPlayerPositions(), { refreshInterval: 10_000, initialData: [] });
  const bans = useApiSWR('api/players/bans', () => apiClient.getBans(), { refreshInterval: 15_000, initialData: [] });
  const connections = useApiSWR('api/players/connections', () => apiClient.getConnectionHistory(), { refreshInterval: 30_000, initialData: [] });
  const allowlist = useApiSWR('api/players/allowlist', () => apiClient.getAllowlist(), { refreshInterval: 30_000, initialData: [] });
