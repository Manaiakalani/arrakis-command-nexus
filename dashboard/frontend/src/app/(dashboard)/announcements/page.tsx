...
export default function AnnouncementsPage() {
  ...
  const startEditing = (entry: { id: string; message: string; sender: string; interval_minutes?: number | null; run_at?: string | null; one_shot?: boolean; enabled: boolean }) => {
    setEditingId(entry.id);
    setEditMessage(entry.message);
    setEditSender(entry.sender);
    setEditInterval(entry.interval_minutes ?? 30);
    setEditRunAt(entry.run_at ?? getDefaultRunAt());
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const isOneShot = !!(scheduled.data ?? []).find((e) => e.id === editingId)?.one_shot;
      const payload = isOneShot
        ? { message: editMessage.trim(), sender: editSender.trim() || undefined, run_at: editRunAt, enabled: editEnabled }
        : { message: editMessage.trim(), sender: editSender.trim() || undefined, interval_minutes: editInterval, enabled: editEnabled };
      await apiClient.updateScheduledAnnouncement(editingId, payload);
      toast('Scheduled announcement updated.', 'success');
      setEditingId(null);
      await refreshScheduled();
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Failed to update';
      toast(`Failed to update: ${errorMessage}`, 'error');
    } finally {
      setRowBusyId(null);
    }
  };
      ...
      <div className="glass-panel p-6">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <button type="button" className="dune-button" onClick={() => void handleSend()} disabled={!canSend}>
            <Megaphone aria-hidden="true" className="mr-2 h-4 w-4" /> Send announcement
          </button>
          <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-th-border-m bg-th-bg/50 px-3 py-3">
            <select className="dune-input w-full sm:min-w-[120px] sm:w-auto" value={minutes} onChange={(event) => setMinutes(Number(event.target.value))} aria-label="Pre-restart warning minutes">
            ...
            <button type="button" className="dune-button-muted" onClick={() => void handlePreRestart()} disabled={busy}>
              <RotateCcw aria-hidden="true" className="mr-2 h-4 w-4" /> Pre-restart warning
            </button>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-[1fr_1fr_auto]">
          ...
          <div className="flex items-end gap-2">
            <button
              ...
              disabled={wisdomBusy}
            >
              <CalendarClock aria-hidden="true" className="mr-2 h-4 w-4" /> Start scheduler
            </button>
            <button
              type="button"
              ...
              disabled={wisdomBusy}
            >
              <Zap aria-hidden="true" className="mr-2 h-4 w-4" /> Send one now
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel overflow-hidden">
        <div className="flex flex-col gap-4 border-b border-th-border-m/80 p-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h2 className="mt-1 inline-flex items-center gap-2 text-xl font-semibold text-th-text"><CalendarClock aria-hidden="true" className="h-5 w-5 text-amber-600 dark:text-amber-300" /> Scheduled announcements</h2>
        ...
        <div className="border-b border-th-border-m/80 p-5">
          ...
          <div className="mt-4 flex justify-end">
            <button type="button" className="dune-button" onClick={() => void handleCreateScheduled()} disabled={!canSchedule}>
              <CalendarClock aria-hidden="true" className="mr-2 h-4 w-4" /> Create scheduled announcement
            </button>
          </div>
        </div>
                ...
                return (
                  <tr key={entry.id} className="align-top text-th-text-s">
                    <td className="px-5 py-4">
                      {isEditing ? (
                        <div className="space-y-2">
                          <input type="text" className="dune-input w-full text-sm" value={editMessage} onChange={(e) => setEditMessage(e.target.value)} placeholder="Message" />
                          <input type="text" className="dune-input w-full text-sm" value={editSender} onChange={(e) => setEditSender(e.target.value)} placeholder="Sender" />
                        </div>
                      ) : (
                        <div className="max-w-2xl space-y-1">
                          <p className="text-th-text">{entry.message === '__WISDOM__' ? <span className="inline-flex items-center gap-1.5"><Sparkles aria-hidden="true" className="h-3.5 w-3.5 text-amber-400" /> Words of Wisdom <span className="text-th-text-m">(random quotes)</span></span> : previewMessage(entry.message)}</p>
                    ...
                    <td className="px-5 py-4">
                      {isEditing ? (
                        entry.one_shot ? (
                          <input type="datetime-local" className="dune-input text-sm" value={editRunAt} onChange={(e) => setEditRunAt(e.target.value)} />
                        ) : (
                          <div className="flex items-center gap-2">
                            <span className="text-xs text-th-text-m">Every</span>
                            <input type="number" className="dune-input w-20 text-sm" min={1} value={editInterval} onChange={(e) => setEditInterval(Number(e.target.value))} />
                            <span className="text-xs text-th-text-m">min</span>
                          </div>
                        )
                      ) : (
                        <div className="flex items-start gap-2 text-th-text-m">
                          {entry.one_shot ? <CalendarClock aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0" /> : <Repeat aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0" />}
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-end gap-2">
                        {isEditing ? (
                          <>
                            <button type="button" className="dune-button px-3 py-2 text-xs" onClick={() => void handleSaveEdit()} disabled={isBusy}>Save</button>
                            <button type="button" className="dune-button-muted px-3 py-2 text-xs" onClick={() => setEditingId(null)}>Cancel</button>
                          </>
                        ) : (
                          <>
