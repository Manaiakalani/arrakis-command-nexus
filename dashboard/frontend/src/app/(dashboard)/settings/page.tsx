...
export default function SettingsPage() {
  ...
  const handleChangeRole = useCallback(async (id: number, role: string) => {
    try {
      await apiClient.updateAdmin(id, { role });
      await admins.refetch();
      toast(`Role updated to ${role}.`, 'success');
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Failed to update role.';
      toast(`Failed to update role: ${message}`, 'error');
    }
  }, [admins, toast]);
  ...
  const handleSaveIdentity = useCallback(async (payload: { worldName: string; externalAddress: string }) => {
    setSavingIdentity(true);
    try {
      const result = await apiClient.updateServerIdentity(payload);
      await serverIdentity.refetch();
      const restartedCount = result.restarted?.length ?? 0;
      toast(`Server identity saved. ${restartedCount} game server(s) restarted.`, 'success');
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Failed to update server identity.';
      toast(`Failed to update server identity: ${message}`, 'error');
    } finally {
      setSavingIdentity(false);
    }
  }, [serverIdentity, toast]);
    ...
    <div className="space-y-6">
      ...
      <div className="glass-panel p-5">
        ...
        <p className="mb-4 text-sm text-th-text-m">
          ...
          Saving recreates the game-server containers so the new values take effect, which briefly
        ...
        <form
          ...
          onSubmit={(e) => {
            ...
            handleSaveIdentity({
              worldName: ((fd.get('worldName') as string) ?? '').trim(),
              externalAddress: ((fd.get('externalAddress') as string) ?? '').trim() || 'auto',
            });
          }}
        >
          <div>
            <label htmlFor="worldName" className="block text-sm font-medium text-th-text-s">In-game server name</label>
            <input
              id="worldName"
              name="worldName"
              maxLength={64}
              className="dune-input mt-1 w-full"
              defaultValue={serverIdentity.data?.worldName ?? ''}
              key={`wn-${serverIdentity.data?.worldName}`}
              disabled={savingIdentity}
            />
            <p className="mt-1 text-xs text-th-text-m">
              Shown in the in-game server browser (WORLD_NAME). Apostrophes, double quotes, and the | character are not allowed.
            </p>
          </div>
          <div>
            <label htmlFor="externalAddress" className="block text-sm font-medium text-th-text-s">Broadcast address</label>
            <input
              id="externalAddress"
              name="externalAddress"
              className="dune-input mt-1 w-full"
              placeholder="auto"
              defaultValue={serverIdentity.data?.externalAddress ?? 'auto'}
              key={`ea-${serverIdentity.data?.externalAddress}`}
              disabled={savingIdentity}
            />
            <p className="mt-1 text-xs text-th-text-m">
              Public IP or hostname players connect to (EXTERNAL_ADDRESS). Use <span className="font-mono">auto</span> to
              detect it automatically. An incorrect value can make the server unreachable.
            </p>
          </div>
          <button type="submit" className="dune-button" disabled={savingIdentity}>
            <Save className="mr-2 h-4 w-4" aria-hidden="true" /> {savingIdentity ? 'Saving\u2026' : 'Save and restart servers'}
          </button>
        </form>
      </div>
        ...
        <section className="glass-panel p-5">
          ...
          >
            ...
            <button type="submit" className="dune-button" disabled={saving === 'general'}>
              <Save className="mr-2 h-4 w-4" aria-hidden="true" /> {saving === 'general' ? 'Saving\u2026' : 'Save general'}
            </button>
          </form>
        </section>
          ...
          >
           ...
           <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
            ...
            <button type="submit" className="dune-button" disabled={saving === 'integrations'}>
              <Save className="mr-2 h-4 w-4" aria-hidden="true" /> {saving === 'integrations' ? 'Saving\u2026' : 'Save integrations'}
            </button>
          </form>
        </section>
        ...
        <section className="glass-panel p-5">
          ...
          >
            ...
            <button type="submit" className="dune-button" disabled={saving === 'appearance'}>
              <Save className="mr-2 h-4 w-4" aria-hidden="true" /> {saving === 'appearance' ? 'Saving\u2026' : 'Save appearance'}
            </button>
          </form>
        </section>
      </div>
      ...
      <div className="glass-panel overflow-hidden">
        <div className="border-b border-th-border-m/80 p-5">
          ...
          <p className="mt-2 text-sm text-th-text-m">
            Track who has access to the Command Nexus. Assign roles to control access levels.
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            <span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-300">Operator — full control</span>
            <span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-amber-700 dark:text-amber-200">Editor — can change settings</span>
            <span className="rounded-full border border-sky-500/30 bg-sky-500/10 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-sky-700 dark:text-sky-300">Viewer — read-only</span>
          </div>
        </div>

        <div className="divide-y divide-th-border-m/80">
          {(admins.data ?? []).map((admin) => {
            const roleBadge = admin.role === 'viewer'
            ...
            return (
              <div key={admin.id} className="flex items-center justify-between p-5">
                ...
                <div className="flex items-center gap-2">
                  <select
                    value={admin.role}
                    onChange={(e) => void handleChangeRole(admin.id, e.target.value)}
        ...
        <div className="border-t border-th-border-m/80 p-5">
          ...
          >
            ...
            <div>
              <label htmlFor="newAdminRole" className="block text-sm font-medium text-th-text-s">Role</label>
              <select
                id="newAdminRole"
                className="dune-input mt-1"
                value={newAdminRole}
