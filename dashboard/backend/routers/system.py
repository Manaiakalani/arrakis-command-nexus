...
_DEGRADED_CPU_THRESHOLD = 95
_DEGRADED_MEMORY_THRESHOLD = 90
...
def _build_uptime_payload(snapshots: list[Any], duration: timedelta, interval_seconds: int) -> dict[str, Any]:
    ...
    return {
        ...
        ][-50:],  # Cap at 50 most recent incidents to prevent frontend freeze
    }
...
async def get_system_history(
    ...
    duration = timedelta(hours=hours) if hours is not None else _parse_range_to_duration(time_range)
    history = await request.app.state.metrics_service.get_history(duration=duration)
    points = [_format_snapshot(snapshot) for snapshot in history]

    # Downsample to prevent frontend freeze on large ranges.
    max_points = 500
    total = len(points)
    if total > max_points:
        ...
        idx = 0
        while idx < total:
            bucket = points[idx : idx + bucket_size]
            avg_point = {k: bucket[0][k] for k in bucket[0]}
            for key in ("cpu", "memory", "disk"):
                vals = [p.get(key, 0) for p in bucket if isinstance(p.get(key), (int, float))]
                if vals:
                    avg_point[key] = round(sum(vals) / len(vals), 2)
            avg_point["timestamp"] = bucket[len(bucket) // 2].get("timestamp", avg_point.get("timestamp"))
            downsampled.append(avg_point)
            idx += bucket_size
        points = downsampled

    return {"range": time_range, "points": points}
...
_RESOURCE_VARS: dict[str, dict[str, str]] = {
    ...
    "MEM_LIMIT_RMQ": {
        ...
        "description": "RAM limit for the RabbitMQ message broker. Handles inter-service communication. Must be at least 1g - lower values trigger system_memory_high_watermark alarms during travel that block publishers and cause client P83 errors.",
        "category": "infrastructure",
        "default": "1g",
    },
    "MEM_LIMIT_DIRECTOR": {
...
# ----------------------------------------------------------------------------
# Graceful host shutdown preparation
# ----------------------------------------------------------------------------
# This endpoint runs the in-game-visible portion of a shutdown: warning
# announcements, a final backup, then stopping the GAME containers. It does
# NOT stop the dashboard (the dashboard is running this code) and does NOT
# power off the host. The matching CLI (`dune shutdown-host`) takes over from
# here to bring the dashboard down and run `sudo shutdown` from the host.
import asyncio as _asyncio  # noqa: E402  - imported here to keep the addition self-contained


class PrepareShutdownRequest(BaseModel):
    warning_minutes: int = 5
    skip_backup: bool = False
    stop_game_servers: bool = True


_GAME_CONTAINER_NAMES = (
    "dune-awakening-survival_1-1",
    "dune-awakening-overmap-1",
    "dune-awakening-deepdesert_1-1",
)
_INFRA_CONTAINER_NAMES = (
    "dune-awakening-director-1",
    "dune-awakening-gateway-1",
    "dune-awakening-text-router-1",
    "dune-awakening-rmq-auth-shim-1",
    "dune-awakening-game-rmq-1",
    "dune-awakening-admin-rmq-1",
    "dune-awakening-partition-repair-1",
    "dune-awakening-seed-guardian-1",
)


@router.post("/system/prepare-shutdown")
async def prepare_shutdown(payload: PrepareShutdownRequest, request: Request) -> dict[str, Any]:
    """Run the safe-to-do-from-the-dashboard portion of a host shutdown.

    Phase 1 (this endpoint, ~payload.warning_minutes + ~30s):
      - Sends in-game warnings at warning_minutes / 1 minute / 10s.
      - At T-0: creates a final full backup, then stops the game containers
        (and infra containers like director/gateway/RMQ in dependency order).
      - Leaves PostgreSQL, the docker socket proxy, and the dashboard itself
        running so the operator can still see what happened.

    Phase 2 (operator runs `dune shutdown-host` from the host):
      - Stops PostgreSQL + remaining containers via `docker compose down`.
      - Optionally runs `sudo shutdown -h now`.

    The split exists because the dashboard runs INSIDE a container — if it
    stopped the whole stack including itself, docker would kill it mid-flush.
    """
    if payload.warning_minutes < 0 or payload.warning_minutes > 60:
        raise HTTPException(status_code=400, detail="warning_minutes must be between 0 and 60")

    docker_service = request.app.state.docker_service
    backup_service = request.app.state.backup_service
    announce_service = request.app.state.announce_service
    discord_service = getattr(request.app.state, "discord_service", None)

    # Background task so the API call returns immediately. The frontend can
    async def _run_real():
        ...
        async def step(msg: str) -> None:
            logger.info("[shutdown] %s", msg)
            _SHUTDOWN_STATE["details"].append({
        ...
        try:
            if discord_service is not None:
                try:
                    await discord_service.enqueue(
                        "scheduled_restart",
                        f"Server shutdown in **{payload.warning_minutes} minute(s)**.\n"
                        f"Players will be disconnected; host will power off.\n"
                        f"Final backup runs automatically.",
                        title="🛑 Host Shutdown Starting",
                    )
                except Exception:  # noqa: BLE001
                    logger.exception("Failed to enqueue Discord shutdown notification")

            # Build a list of (seconds_from_start, message) waypoints.
            total_seconds = payload.warning_minutes * 60
            waypoints: list[tuple[int, str]] = []
            if payload.warning_minutes >= 5:
                waypoints.append((0, f"Server shutting down in {payload.warning_minutes} minutes for maintenance."))
            elif payload.warning_minutes > 0:
                waypoints.append((0, f"Server shutting down in {payload.warning_minutes} minute(s) for maintenance."))
            if payload.warning_minutes >= 2:
                waypoints.append((total_seconds - 60, "Server shutting down in 1 minute. Please log out safely."))
            if total_seconds >= 10:
                waypoints.append((total_seconds - 10, "Server shutting down in 10 seconds. Goodbye!"))

            elapsed = 0
            for at_seconds, message in waypoints:
                gap = at_seconds - elapsed
                if gap > 0:
                    await _asyncio.sleep(gap)
                    elapsed = at_seconds
                _SHUTDOWN_STATE["phase"] = f"warning_T-{total_seconds - elapsed}s"
                await step(f"Announcing: {message}")
                try:
                    await _asyncio.to_thread(announce_service.send_announcement, message)
                except Exception:  # noqa: BLE001
                    logger.exception("Announcement send failed (continuing)")

            # Final wait until T-0.
            if total_seconds - elapsed > 0:
                await _asyncio.sleep(total_seconds - elapsed)

            if not payload.skip_backup:
                _SHUTDOWN_STATE["phase"] = "backup"
                await step("Creating final pre-shutdown backup (full scope)")
                try:
            ...
            if payload.stop_game_servers:
                ...
                for name in _GAME_CONTAINER_NAMES:
                    try:
                        await docker_service.stop_container(name)
                        await step(f"Stopped {name}")
                    except KeyError:
            ...
            if discord_service is not None:
                try:
                    await discord_service.enqueue(
                        "scheduled_restart",
                        "Game and infra containers stopped. Run `dune shutdown-host --confirm` from the host to power off.",
    ...
    _asyncio.create_task(_run_real(), name="shutdown-phase-1")
    return {
        "status": "started",
        "warning_minutes": payload.warning_minutes,
        "skip_backup": payload.skip_backup,
        "stop_game_servers": payload.stop_game_servers,
        "message": "Shutdown phase 1 running in background. Poll GET /system/shutdown-status.",
    }


_SHUTDOWN_STATE: dict[str, Any] = {"phase": "idle"}


@router.get("/system/shutdown-status")
async def get_shutdown_status() -> dict[str, Any]:
    """Return the current shutdown phase + recent log entries."""
    return dict(_SHUTDOWN_STATE)
