...
class DockerService:
    def __init__(self, base_url: str | None = None) -> None:
        ...
        self.role_patterns: dict[str, tuple[str, ...]] = {
            ...
            "overmap": ("overmap",),
            "survival": ("survival", "sietch", "hagga"),
            "deep_desert": ("deep_desert", "deepdesert"),
            "arrakeen": ("arrakeen",),
            "harko_village": ("harko_village", "harko"),
            "story": ("proces_verbal", "testing_carthag", "testing_hephaestus", "testing_waterfat"),
            "dashboard": ("dashboard",),
        }
        self.critical_roles = {"gateway", "director", "postgres"}
        self._map_roles = {"overmap", "survival", "deep_desert", "arrakeen", "harko_village", "story"}
    ...
    async def list_map_statuses(self) -> list[MapStatus]:
        services = await self.list_containers()
        map_services = [
            s for s in services if self._map_role(s.name) in self._map_roles
        ]
        if not map_services:
            return []
