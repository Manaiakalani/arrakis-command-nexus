...
backup_database() {
  dump_file="$BACKUP_DIR/dune-db__${timestamp}.sql.gz"
  echo "Creating PostgreSQL backup: $dump_file"
  pg_dump -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" -U "$POSTGRES_USER" "$POSTGRES_DB" \
    | gzip > "$dump_file"
  created_files="$created_files $dump_file"
}

backup_config() {
  config_archive="$BACKUP_DIR/dune-config__${timestamp}.tar.gz"
  ...
  created_files="$created_files $config_archive"
}
...
for f in $created_files; do
  file_names="${file_names:+$file_names,}$(basename "$f")"
done
cat > "$meta_file" <<EOF
scope=$scope
timestamp=$timestamp
...
for f in $created_files "$meta_file"; do
