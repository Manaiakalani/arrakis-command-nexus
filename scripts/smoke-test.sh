...
check_api "/api/watchdog/status" "Watchdog status"
check_api "/api/watchdog/crashes" "Watchdog crash history"
...
# Match real problems by log level (uppercase ERROR/CRITICAL/FATAL) plus
# tracebacks and unhandled-exception markers. A case-insensitive 'error' match
# produced false positives from benign INFO lines (e.g. pika's "error=None" and
# "Normal shutdown" on clean AMQP closes).
api_errors="$(docker logs dune-awakening-dashboard-api-1 --tail 100 2>&1 | \
  grep -cE '(^|[[:space:]])(ERROR|CRITICAL|FATAL)([: ]|$)|Traceback|Unhandled exception|Exception in ' || true)"
