...
log_step "Network status"
{
  ...
  echo ""
  echo "=== UDP socket errors (rubberbanding: watch 'receive buffer errors') ==="
  netstat -su 2>/dev/null | grep -iE 'receive buffer errors|packet receive errors|InErrors' \
    || nstat -az 2>/dev/null | grep -iE 'UdpRcvbufErrors|UdpInErrors' \
    || echo "(netstat/nstat unavailable)"
  echo ""
  echo "=== NIC ring + offload (default-route interface) ==="
  nic_dev="$(ip route get 1.1.1.1 2>/dev/null | awk '{print $5; exit}')" || true
  if [ -n "${nic_dev:-}" ]; then
    echo "interface: ${nic_dev}"
    ethtool -g "${nic_dev}" 2>/dev/null || echo "(ethtool -g unavailable)"
    ethtool -k "${nic_dev}" 2>/dev/null | grep -iE 'generic-receive-offload|udp-gro|large-receive-offload' || true
  else
    echo "(default-route interface not detected)"
  fi
  echo ""
