...
funcom_token="$(read_funcom_token || true)"
if [[ -z "$funcom_token" ]]; then
  check 'Funcom token is present.' 'Funcom token is missing. Set FLS_SECRET in .env or write secrets/funcom-token.txt.' '1'
elif [[ "$funcom_token" == change-me* ]]; then
  check 'Funcom token is present.' 'Funcom token is still the change-me placeholder. Set a real FLS_SECRET in .env (or secrets/funcom-token.txt).' '1'
else
