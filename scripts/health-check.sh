...
if have_command docker && (service_exists admin-rmq 2>/dev/null || service_exists game-rmq 2>/dev/null); then
  ...
  rabbitmq_user="$(strip_wrapping_quotes "${DUNE_RMQ_MANAGEMENT_USER:-${RABBITMQ_DEFAULT_USER:-}}")"
  rabbitmq_pass="$(strip_wrapping_quotes "${DUNE_RMQ_MANAGEMENT_PASSWORD:-${RABBITMQ_DEFAULT_PASS:-}}")"
