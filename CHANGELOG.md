...
- Full ornithopter kits grant enough wings to assemble: Scout 4, Assault 6, Carrier 8 plus 2 side hulls and 2 tail hulls
...
- Grant catalog now covers the remaining live named IDs: CHOAM Heavy Mk1/3/4 sets, CHOAM Light Mk6 casing, Efficient Stillsuit Mk5, HarkAr6, AtreLMG4, RocketLauncher_3, unique sidearms/shotguns, mid-tier vehicle parts, missing tools/emotes/schematics (including three live schematic IDs newly named in `KNOWN_TEMPLATES`)
- Dashboard pages under the shell use `h2` so the server name stays the only `h1`. Public status shows the real Funcom build tag instead of a fake 100% availability
- Connection tracker loop lives in `player_tracker.track_player_connections` with the join/leave math
- Dashboard frontend stays on ESLint 9 (`^9.39.5`). Dependabot’s ESLint 10 bump in #116 is valid for `eslint-config-next`’s own peer (`>=9`), but the plugins that package still vendors (`eslint-plugin-import`, `eslint-plugin-react`, `eslint-plugin-jsx-a11y`) only declare ESLint 9 and spam `npm ci` on every image build
...
- **In-game rubberbanding and S2S `NumOutRec 2047` mesh crashes came back** on a live standard-lean host even with host networking, CPU pinning, and 16 MB socket buffers in place. UE5's default `MaxClientRate` / `MaxInternetClientRate` (~10–15 KB/s) throttled both the player `IpNetDriver` (ack window of 256 overflowed; clients rubberband) and the inter-server `IgwNetDriver` (reliable channel filled, the map marked peers — or itself — dead, then `DuneWorldPartitioner.cpp` SIGSEGV'd). Funcom's documented next test after socket buffers was `IgwNetDriver:MaxClientRate=0` only; that is now the shipped baseline, plus a finite player cap of `100000` (not `0` — unlimited player bandwidth floods initial replication and clients fail with error 324). Applied in `UserEngine.ini` and on every game-server command in the basic / standard / standard-lean / full compose files. CI and `dune` preflight run `scripts/assert-net-rate-flags.sh` so dropping a flag is a hard fail rather than a silent rubberband regression
