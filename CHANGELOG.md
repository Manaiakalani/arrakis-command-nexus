...
- The Uptime Kuma push token was returned in full to any authenticated caller, including viewers, via `GET /settings`, `GET /settings/{section}` and the settings export. Non-operators now receive a placeholder, and saving that placeholder back restores the stored value rather than overwriting it (#53)
- Failed sign-in attempts are now throttled per username as well as per client IP. The client IP is derived from `X-Forwarded-For`, so an IP-only window could be sidestepped by varying the header. The attempt table is also swept and capped, so it cannot grow without bound
